import styled from 'styled-components';

export const Scategory = 'mt-24 mb-4 flex flex-wrap justify-center';

export const SDisplayCrd = `min-w-full min-h-full flex flex-wrap justify-around items
items-eviny  mb-8`;

export const footer = `min-w-full px-8 py-1  flex mt-12 bg-gray-100 border-t 
transition-opacity justify-between
border-gray-50 shadow-md fixed bottom-0 left-0 z-20 items-center`;

export const barShareButton = `w-full h-16 flex justify-center items-center
border-r border-gray-300`;

export const StyleCarFooter = 'w-full flex items-center';

export const SizePage = styled.div`background: transparent;
  width: 360px;
`;
